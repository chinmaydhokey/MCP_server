...
/**
 * SQLite (libsql) schema — 17 tables, identical names and columns to `./pg.ts` (enforced by
 * `test/conformance.test.ts`).
 *
 * Type mapping (ADR-0006): ids are UUIDv7 strings in `text`; timestamps are integer epoch milliseconds;
 * JSON is `text` with Drizzle `{ mode: 'json' }`; booleans are `integer { mode: 'boolean' }`; enums are
 * `text` typed with the shared arrays from `./enums.ts`. No column has a DB-side default for ids or
 * timestamps — the application supplies them so both dialects behave identically.
 */
...
export const testCase = sqliteTable(
  'test_case',
  {
    ...
    currentRevisionId: id('current_revision_id').references((): AnySQLiteColumn => testCaseRevision.id),
    owners: json('owners'),
    tags: json('tags'),
    platforms: json('platforms'),
    status: text('status', { enum: TEST_CASE_STATUS }).notNull().default('active'),
    firstSeenRunId: id('first_seen_run_id').references((): AnySQLiteColumn => run.id),
...
export const testCaseRevision = sqliteTable(
  'test_case_revision',
  {
    id: id().primaryKey(),
    ...
    parentRevisionId: id('parent_revision_id').references((): AnySQLiteColumn => testCaseRevision.id),
...
export const testStep = sqliteTable(
  'test_step',
  {
    id: id().primaryKey(),
...
export const stepFingerprint = sqliteTable(
  'step_fingerprint',
  {
    id: id().primaryKey(),
    projectId: id('project_id')
    ...
    cropArtifactId: id('crop_artifact_id').references((): AnySQLiteColumn => artifact.id),
    capturedAtRunId: id('captured_at_run_id').references((): AnySQLiteColumn => run.id),
...
export const stepResult = sqliteTable(
  'step_result',
  {
    id: id().primaryKey(),
    ...
    healProposalId: id('heal_proposal_id').references((): AnySQLiteColumn => healProposal.id),
    oracle: text('oracle', { enum: ORACLE_KIND }),
    failureCategory: text('failure_category', { enum: FAILURE_CATEGORY }),
    errorSignature: text('error_signature'),
    errorMessage: text('error_message'),
    snapshotArtifactId: id('snapshot_artifact_id').references((): AnySQLiteColumn => artifact.id),
    screenshotArtifactId: id('screenshot_artifact_id').references((): AnySQLiteColumn => artifact.id),
...
export const healProposal = sqliteTable(
  'heal_proposal',
  {
    id: id().primaryKey(),
    projectId: id('project_id')
...
export const flakyStat = sqliteTable(
  'flaky_stat',
  {
    id: id().primaryKey(),
    projectId: id('project_id')
...
export const quarantine = sqliteTable(
  'quarantine',
  {
    id: id().primaryKey(),
    projectId: id('project_id')
...
export const coverageMap = sqliteTable(
  'coverage_map',
  {
    id: id().primaryKey(),
    projectId: id('project_id')
