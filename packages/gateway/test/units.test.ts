...
import { parseConfig } from '@qa-brain/core';
...
describe('expandEnv', () => {
  it('expands ${VAR} and ${VAR:-default} and reports expanded secrets', () => {
    const { value, expanded } = expandEnv(
      { a: '${FOO}', b: '${MISSING:-dflt}', c: ['x${FOO}y'], n: 1 },
      { FOO: 'secret1' },
    );
...
describe('config coercion from the environment', () => {
  it('accepts numeric strings for numeric fields', () => {
    const c = parseConfig({ http: { port: '9999' }, server: { callTimeoutMs: '1500' } });
    expect(c.http.port).toBe(9999);
    expect(c.server.callTimeoutMs).toBe(1500);
  });

  it('treats the string "false" as false, never as truthy', () => {
    // z.coerce.boolean() would turn "false" into true and silently disable the auth requirement.
    const c = parseConfig({ http: { allowUnauthenticated: 'false' }, server: { exposeStubs: 'true' } });
    expect(c.http.allowUnauthenticated).toBe(false);
    expect(c.server.exposeStubs).toBe(true);
    expect(parseConfig({ http: { allowUnauthenticated: '0' } }).http.allowUnauthenticated).toBe(false);
    expect(parseConfig({ http: { allowUnauthenticated: true } }).http.allowUnauthenticated).toBe(true);
  });

  it('rejects values that are neither boolean nor a known boolean spelling', () => {
    expect(() => parseConfig({ http: { allowUnauthenticated: 'maybe' } })).toThrow();
    expect(() => parseConfig({ http: { port: 'not-a-port' } })).toThrow();
  });
});
