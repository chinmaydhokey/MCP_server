...
| T12 | Handles (`rn_…`, `bh_…`) treated as authentication | S | Handles are `<kind>_<uuidv7>`, stored with `owner_api_key_id`, TTL-swept, owner-checked on every use | M0 schema, M1 enforce | Schema + `run_id` owner check |
