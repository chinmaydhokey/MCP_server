...
- **What `pnpm smoke` asserts** (`apps/qa-brain/test/smoke.e2e.test.ts`, two suites): the Playwright MCP child reports itself as `Playwright 1.62.1` and is healthy; `tools/list` is sorted, within the 25-tool budget, every name matches the Claude tool-name regex, and `web_run_code_unsafe`, `web_evaluate` and the hidden `web_tabs` are absent; `web_navigate` to `examples/static-site` returns the page summary, `web_snapshot` contains `[ref=` and the "Add to cart" button, `web_click` on that ref makes `web_verify_text_visible` find "Added to cart"; the SQLite `action_log` has a `web_navigate` row with `args_shape`, `args_redacted` and a valid `traceparent`; the blocked tools stay refused even through `qa_call_tool`; and the child process tree is gone after `client.close()`. The second suite repeats the list-and-navigate path against the **built binary** (`apps/qa-brain/dist/cli.js serve --transport stdio`) so the packaged artifact is covered too.

  Note that `browser_navigate` answers with a page *summary* (title, URL, links to the snapshot file it wrote under `--output-dir`); the accessibility tree with `[ref=eN]` targets comes from `browser_snapshot`. Scratch directories are created under the gitignored `.qa-brain/`, which is what the failure artifact uploads.
