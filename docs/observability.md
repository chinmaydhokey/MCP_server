...
`error.type` holds the gateway's error code (`timeout`, `upstream_degraded`, `protocol_error`, `cancelled`, `tool_not_allowed`) and the span status is `ERROR` whenever the result carries `isError: true`, matching rows with `action_log.is_error = 1`.
...
- [ADR-0013](./adr/0013-observability-otel-semconv.md) — decision record
